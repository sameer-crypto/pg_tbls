...
| in_saldn | CREATE INDEX in_saldn ON public.sale_deliverynote USING btree (deliverynoteid, deliverynotedate, iscancelled, branchid, isauthorized, storeid) WHERE ((branchid = 1) AND (iscancelled = false) AND (isauthorized = true)) |
| ui_deliverynote_no | CREATE UNIQUE INDEX ui_deliverynote_no ON public.sale_deliverynote USING btree (branchid, dltype, ((deliverynotedate)::date), deliverynoteno) WHERE (deliverynoteid > 473965) |
